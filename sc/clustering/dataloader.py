...
def get_dataloaders(csv_fn, batch_size, train_val_test_ratios=(0.8, 0.2, 0), n_aux=0,
                    save_split_csvs=True, split_csv_prefix=None):
    transform_list = transforms.Compose([ToTensor()])

    ds_train, ds_val, ds_test = [
        AuxSpectraDataset(csv_fn, p, train_val_test_ratios, transform=transform_list, n_aux=n_aux)
        for p in ["train", "val", "test"]
    ]

    if save_split_csvs:
        base = Path(csv_fn)
        if split_csv_prefix is None:
            split_csv_prefix = str(base.with_suffix(""))  # "/path/data" from "/path/data.csv"

        for name, ds in [("train", ds_train), ("val", ds_val), ("test", ds_test)]:
            ids = ds.get_ids()
            out_fn = f"{split_csv_prefix}_split_{name}.csv"
            pd.DataFrame({"id": ids}).to_csv(out_fn, index=False)
