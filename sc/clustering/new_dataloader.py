...
def make_split_bins(train_val_test_ratios=(0.8, 0.2, 0.0), n_bins=128, seed=22):
    portion_options = ["train", "val", "test"]
    assert len(train_val_test_ratios) == 3

    counts = [
        int(train_val_test_ratios[0] * n_bins),
        int(train_val_test_ratios[1] * n_bins),
        int(train_val_test_ratios[2] * n_bins),
    ]
    counts[0] += (n_bins - sum(counts))  # remainder -> train
...
def save_split_ids(full_df: pd.DataFrame, dfs_by_split: dict, split_bins: dict,
    ...
    Saves:
      ...
      ./<prefix>_ids_train.csv, ./<prefix>_ids_val.csv, ./<prefix>_ids_test.csv

    The *_ids_*.csv will contain your MultiIndex levels as columns
    (e.g., mpid / id / site / jjj depending on your CSV indexing).
    """
    ...
    _atomic_write_text(out_dir / f"{prefix}_manifest.json", json.dumps(manifest, indent=2))

    for split_name, df_split in dfs_by_split.items():
        idx_df = df_split.index.to_frame(index=False)  # robust for MultiIndex or Index
...
def get_dataloaders(csv_fn, batch_size, train_val_test_ratios=(0.8, 0.2, 0.0),
                    n_aux=0, n_bins=128, seed=22, save_split_files=True, out_dir="."):
    transform_list = transforms.Compose([ToTensor()])

    # read ONCE
    full_df = pd.read_csv(csv_fn, index_col=[0, 1], comment="#")

    # split plan computed ONCE
    split_bins = make_split_bins(train_val_test_ratios, n_bins=n_bins, seed=seed)
    ...
    if save_split_files:
        save_split_ids(
            ...
            out_dir=out_dir,         # guaranteed in ./ via resolve() + mkdir
            prefix="split",
            extra_meta={
